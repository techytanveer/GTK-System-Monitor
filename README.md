...
## 3. Security
Memory Safety: Built using Modern C++ (Smart Pointers) to eliminate manual memory management and prevent leaks or dangling pointers.

Minimal Privileges: Shift operates entirely in user-space, requiring no sudo or elevated permissions to read system stats.
...
IDataProvider: An abstract interface allowing for easy swapping of data sources (e.g., switching from libgtop to a custom /proc parser).

Clear Hierarchy: Logical separation between Core (logic) and UI (presentation).
